...
class Lemmatiser:
    ...
    def lemmatise(self, word):
        ...
        return sorted(
            {analysis.analysis.split("+")[0] for analysis in self.analyse(word)}
        )
...
class SmeLemmatiser(Lemmatiser):
    ...
    classifications = {
        "verb_abessive": Classification(re.compile(r".*VABess.*"), "+V+VABess"),
        "verb_gerundium": Classification(re.compile(r".*\+Ger.*"), "+V+Ger"),
        "verb_action_form": Classification(
            re.compile(r".*\+V(\+(IV|TV))\+Actio.*"), "+V+Actio+Nom"
        ),
        "verb_genitive": Classification(
            re.compile(r".*\+V(\+(IV|TV))\+VGen.*"), "+V+VGen"
        ),
        "adjective_ordinal_number": Classification(
            re.compile(r".*\+A\+Ord.*"), "+A+Ord+Sg+Nom"
        ),
        "attributive": Classification(re.compile(r".*\+N\+Attr.*"), "+N+Attr"),
        "propernoun": Classification(re.compile(r".*\+N\+Prop.*"), "+N+Prop+Sg+Nom"),
        "collective_numeral": Classification(
            re.compile(r".*\+N\+Coll.*"), "+N+Coll+Sg+Nom"
        ),
        "noun_abbreviation": Classification(
            re.compile(r".*\+N\+ABBR.*"), "+N+ABBR+Sg+Nom"
        ),
        "nomen_agentis": Classification(
            re.compile(r".*\+N\+NomAg.*"), "+N+NomAg+Sg+Nom"
        ),
        "noun_acronym": Classification(re.compile(r".*\+N\+ACR.*"), "+N+ACR+Sg+Nom"),
        "adverb_abbreviation": Classification(
            re.compile(r".*\+Adv\+ABBR.*"), "+Adv+ABBR"
        ),
        "numeral": Classification(re.compile(r".*\+Num\+(Sg|Pl|Ess)"), "+Num+Sg+Nom"),
        "adverb": Classification(re.compile(r".*\+Adv$"), "+Adv"),
        "verb": Classification(
            re.compile(r"^\+V(\+(IV|TV))*\+(Inf|Ind|Imprt|Cond|Pot|PrfPrc|PrsPrc)"),
            "+V+Inf",
        ),
        "adjective": Classification(re.compile(r"^\+A\+(Sg|Pl|Attr|Ess)"), "+A+Sg+Nom"),
        "noun": Classification(
            re.compile(r"^\+N\+(Sg|Pl|Attr|Ess|G3|G7|NomAg)"), "+N+Sg+Nom"
        ),
    }
    ...
    def analysis_to_wordforms(self, analysis):
        ...
        return [
            ATTS.sub("", generated[0]) for generated in self.generator.lookup(analysis)
        ]
    ...
    def generate(self, analysis):
        ...
        return [
            f"{cmp}{generated}"
            for generated in self.analysis_to_wordforms(f"{start}{classified_tags}")
        ]
...
def lemmatiser(language):
    ...
    return Lemmatiser(language) if language != "sme" else SmeLemmatiser(language)
