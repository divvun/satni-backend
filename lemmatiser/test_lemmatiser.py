...
class TestSmeRegexes(unittest.TestCase):
    ...
    def test_adjective(self, adjective_value):
        assert (
            self.lemmatiser.classifications["adjective"].regex.match(adjective_value)
            is not None
        )
    ...
    def test_noun(self, adjective_value):
        assert (
            self.lemmatiser.classifications["noun"].regex.match(adjective_value)
            is not None
        )
    ...
    def test_verb(self, verb_value):
        assert (
            self.lemmatiser.classifications["verb"].regex.match(verb_value) is not None
        )

    @params(
        ("+A+Der/Comp+A+Sg+Nom", "+A+Sg+Nom"), ("+A+Der/Superl+A+Sg+Nom", "+A+Sg+Nom")
    )
    def test_adj_comp_subj(self, test_value, expected):
        assert (
            self.lemmatiser.removable_regex_tags["adjective_comp_superl"].sub(
                "", test_value
            )
            == expected
        )
...
class TestLemmatiser(unittest.TestCase):
    ...
    def test_lemmatiser(self, language, word, exptected_results):
        ...
        assert self.lemmatisers[language].lemmatise(word) == sorted(exptected_results)
